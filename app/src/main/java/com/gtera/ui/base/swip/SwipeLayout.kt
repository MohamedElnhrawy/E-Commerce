...
class SwipeLayout @JvmOverloads constructor(
    ...
    private val mDragEdges =
        LinkedHashMap<DragEdge, View?>()
    private var mShowMode: ShowMode? = null
    private val mEdgeSwipesOffset = FloatArray(4)
    private val mSwipeListeners: MutableList<SwipeListener> =
        ArrayList()
    private val mSwipeDeniers: MutableList<SwipeDenier> =
        ArrayList()
    private val mRevealListeners: MutableMap<View, ArrayList<OnRevealListener>?> =
        HashMap()
    private val mShowEntirely: MutableMap<View, Boolean> =
        HashMap()
    private val mViewBoundCache: MutableMap<View?, Rect> =
        HashMap() //save all children's bound, restore in onLayout
    private var mDoubleClickListener: DoubleClickListener? =
        null
    ...
    private val mDragHelperCallback: ViewDragHelper.Callback = object : ViewDragHelper.Callback() {
        ...
        override fun tryCaptureView(
            ...
            val result =
                child === surfaceView || bottomViews.contains(child)
            if (result) {
                isCloseBeforeDrag =
    ...
    private fun captureChildrenBound() {
        ...
        val views =
            arrayOf(surfaceView, currentBottomView)
        for (child in views) {
    ...
    protected fun dispatchSwipeEvent(
        surfaceLeft: Int,
        surfaceTop: Int,
        dx: Int,
        dy: Int
    ...
    protected fun dispatchSwipeEvent(
        ...
        if (!mSwipeListeners.isEmpty()) {
            ...
            for (l in mSwipeListeners) {
                ...
                l.onUpdate(
                    this@SwipeLayout,
                    surfaceLeft - paddingLeft,
                    surfaceTop - paddingTop
                )
            }
    ...
    private fun checkCanDrag(ev: MotionEvent) {
        ...
        if (dragEdge == DragEdge.Right) {
            var suitable =
                (status == Status.Open && distanceX > mTouchSlop
                        || status == Status.Close && distanceX < -mTouchSlop)
            suitable =
                suitable || status == Status.Middle
    ...
    override fun onInterceptTouchEvent(ev: MotionEvent): Boolean {
        ...
        if (isClickToClose && openStatus == Status.Open && isTouchOnSurface(
                ev
            )
        ) {
            return true
        ...
        if (t is AdapterView<*>) {
            ...
            try {
                ...
                if (view.onItemLongClickListener != null) {
                    handled = view.onItemLongClickListener
                        .onItemLongClick(view, this@SwipeLayout, p, vId)
                }
                if (handled) {
     ...
     */
    protected fun processHandRelease(
        xvel: Float,
        yvel: Float,
        isCloseBeforeDragged: Boolean
    ) {
        ...
        if (currentDragEdge == DragEdge.Left) {
            ...
            if (xvel > minVelocity) close() else if (xvel < -minVelocity) open() else {
                val openPercent = 1f * -surfaceView.left / mDragDistance
                if (openPercent > willOpenPercent) close() else close()
            }
    ...
    fun open(smooth: Boolean = true, notify: Boolean = true) {
        ...
        if (smooth) {
            ...
            if (showMode == ShowMode.PullOut) {
                val bRect =
                    computeBottomLayoutAreaViaSurface(ShowMode.PullOut, rect)
    ...
    fun close(smooth: Boolean = true, notify: Boolean = true) {
        ...
        val dx: Int
        val dy: Int
        if (smooth) mDragHelper.smoothSlideViewTo(
            surfaceView!!,
            paddingLeft,
            paddingTop
        ) else {
    ...
    private fun computeBottomLayoutAreaViaSurface(
        mode: ShowMode,
        surfaceArea: Rect?
    ): Rect {
    ...
    private val currentOffset: Float
        private get() = (if (dragEdge == null) 0 else mEdgeSwipesOffset[dragEdge!!.ordinal]) as Float
    ...
    companion object {
        ...
        private val DefaultDragEdge =
            if (Locale.getDefault().displayLanguage
                    .toLowerCase() == "ar"
            ) DragEdge.Right else DragEdge.Left
    }
    ...
    init {
        ...
        val dragEdgeChoices = a.getInt(
            R.styleable.SwipeLayout_drag_edge,
            if (Locale.getDefault().displayLanguage
                    .toLowerCase() == "ar"
            )   DRAG_LEFT else DRAG_RIGHT
        )
